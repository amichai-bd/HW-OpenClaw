...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="pd_exec",
        help=(
            "after the PD scaffold, run the optional pinned ORFS Nangate45 RTL-to-GDS backend"
        ),
    )
...
def prepare_pd_summary(context: dict) -> None:
    ...
    orfs_completed = bool(context.get("pd_exec_backend") and context.get("orfs_metrics"))
    if orfs_completed:
        metrics = context["orfs_metrics"]
        status = {
            "completed": True,
            "stage": "orfs-rtl-to-gds",
            "quality_level": "nangate45-reference-implementation",
            "reason": "Pinned OpenROAD Flow Scripts completed synthesis, P&R, extraction, timing, GDS, and DRC",
            "timing": "pass" if metrics.get("finish__timing__setup__tns") == 0 and metrics.get("finish__timing__hold__tns") == 0 else "fail",
            "drc": "pass" if context.get("orfs_drc_count") == 0 else "fail",
            "lvs": "not-run-missing-nangate45-reference-deck",
            "extraction": "openrcx-spef-complete",
            "setup_tns": metrics.get("finish__timing__setup__tns"),
            "hold_tns": metrics.get("finish__timing__hold__tns"),
            "setup_worst_slack": metrics.get("finish__timing__setup__ws"),
            "hold_worst_slack": metrics.get("finish__timing__hold__ws"),
            "fmax_hz": metrics.get("finish__timing__fmax"),
            "drc_violations": context.get("orfs_drc_count"),
        }
        limitations = [
            "Nangate45/FreePDK45 is a reference platform, not a manufacturable foundry PDK",
            "KLayout LVS is not run because the pinned ORFS image does not ship the Nangate45 LVS deck it references",
            "LEC is disabled for this pinned WSL-compatible profile; Verilator simulation remains a separate pre-PD check",
            "SVG/PNG review images remain builder-generated; authoritative geometry is the ORFS DEF/GDS output",
        ]
    else:
        status = {
            "completed": True,
            ...
            "reason": (
                ...
                "run with -pd-exec for pinned ORFS Nangate45 implementation"
            ),
        ...
        limitations = [
            ...
            "GDS is deterministic foundation geometry, not output from the pinned ORFS backend",
            "run ./setup --pd then use -pd -pd-exec for real Nangate45 RTL-to-GDS output",
        ]

    backend_summary = {
        key: format_text(value, context) if isinstance(value, str) else value
        for key, value in context["pd_backend"].items()
    }
    summary = {
        "pd": {
            "ip": context["ip"],
...
def collect_filelist_include_dirs(filelist_path: Path, visited: set[Path], include_dirs: set[Path]) -> None:
    ...
    for raw_line in resolved_filelist.read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("+incdir+"):
            include_dirs.add(
                Path(line[len("+incdir+") :].replace("$MODEL_ROOT", str(REPO_ROOT))).resolve()
            )
            continue
        if line.startswith("-F "):
            nested = Path(line[3:].replace("$MODEL_ROOT", str(REPO_ROOT)))
            collect_filelist_include_dirs(nested, visited, include_dirs)


def orfs_workspace_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        relative = resolved.relative_to(REPO_ROOT)
    except ValueError as error:
        raise BuildError(f"ORFS input must be inside the repository: {resolved}") from error
    return str(Path("/workspace") / relative)


def validate_orfs_constraints(context: dict) -> dict:
    constraints = context["pd_constraints"].get("orfs")
    if not isinstance(constraints, dict):
        raise BuildError(
            f"'-pd-exec' requires an 'orfs' mapping in ip.{context['ip']}.pd_constraints"
        )
    require_keys(
        constraints,
        [
        ...
        f"ip.{context['ip']}.pd_constraints.orfs",
    )
    for key in ["die_area_um", "core_area_um"]:
        values = constraints[key]
        if not isinstance(values, list) or len(values) != 4 or not all(is_number(item) for item in values):
            raise BuildError(f"'{key}' must be a four-number list in ip.{context['ip']}.pd_constraints.orfs")
        if values[2] <= values[0] or values[3] <= values[1]:
            raise BuildError(f"'{key}' must define positive lower-left/upper-right bounds")
    if not is_number(constraints["place_density_lb_addon"]):
        raise BuildError("'place_density_lb_addon' must be numeric")
    for key in ["lec_check", "run_drc", "run_lvs"]:
        if not isinstance(constraints[key], bool):
            raise BuildError(f"'{key}' must be boolean in ip.{context['ip']}.pd_constraints.orfs")
    if not isinstance(constraints["lvs_note"], str) or not constraints["lvs_note"].strip():
        raise BuildError("'lvs_note' must be a non-empty string")
    return constraints


def write_orfs_inputs(context: dict, orfs_dir: Path, constraints: dict) -> tuple[Path, Path, Path]:
    generated_filelist = Path(context["generated_rtl_filelist"])
    sources: set[Path] = set()
    collect_filelist_sources(generated_filelist, set(), sources)
    rtl_sources = sorted(path for path in sources if path.suffix in {".v", ".sv"})
    if not rtl_sources:
        raise BuildError(f"no Verilog/SystemVerilog sources found in {generated_filelist}")
    include_dirs: set[Path] = set()
    collect_filelist_include_dirs(generated_filelist, set(), include_dirs)

    config_path = orfs_dir / "config.mk"
    sdc_path = orfs_dir / "constraint.sdc"
    work_home = orfs_dir / "work"
    orfs_dir.mkdir(parents=True, exist_ok=True)
    workspace_sources = " ".join(orfs_workspace_path(path) for path in rtl_sources)
    workspace_includes = " ".join(orfs_workspace_path(path) for path in sorted(include_dirs))
    die_area = " ".join(str(value) for value in constraints["die_area_um"])
    core_area = " ".join(str(value) for value in constraints["core_area_um"])
    config_lines = [
        f"export DESIGN_NAME = {context['rtl_module']}",
        f"export DESIGN_NICKNAME = hw_openclaw_{context['ip']}",
        f"export PLATFORM = {context['pd_backend']['platform']}",
        "",
        f"export VERILOG_FILES = {workspace_sources}",
        f"export VERILOG_INCLUDE_DIRS = {workspace_includes}",
        f"export SDC_FILE = {orfs_workspace_path(sdc_path)}",
        "",
        f"export DIE_AREA = {die_area}",
        f"export CORE_AREA = {core_area}",
        f"export PLACE_DENSITY_LB_ADDON = {constraints['place_density_lb_addon']}",
        "export TNS_END_PERCENT = 100",
        "export SYNTH_REPEATABLE_BUILD = 1",
        f"export LEC_CHECK = {1 if constraints['lec_check'] else 0}",
    ]
    ...
    sdc_lines = [
        f"current_design {context['rtl_module']}",
        "",
        f"set clk_name {clock}",
        f"set clk_port_name {clock}",
    ...
    sdc_path.write_text("\n".join(sdc_lines) + "\n", encoding="utf-8")
    return config_path, sdc_path, work_home


def copy_required(source: Path, destination: str, artifact_name: str) -> None:
    if not source.is_file():
        raise BuildError(f"missing ORFS {artifact_name}: {source}")
    destination_path = Path(destination)
    destination_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(source, destination_path)


def capture_orfs_artifacts(
    context: dict,
    work_home: Path,
    sdc_path: Path,
    constraints: dict,
) -> dict:
    platform = context["pd_backend"]["platform"]
    nickname = f"hw_openclaw_{context['ip']}"
    result_dir = work_home / "results" / platform / nickname / "base"
    report_dir = work_home / "reports" / platform / nickname / "base"
    log_dir = work_home / "logs" / platform / nickname / "base"
    copy_required(result_dir / "2_1_floorplan.odb.def", context["pd_floorplan_def"], "floorplan DEF")
    copy_required(result_dir / "3_2_place_iop.tcl", context["pd_io_placement_tcl"], "IO placement Tcl")
    copy_required(sdc_path, context["pd_timing_sdc"], "timing SDC")
    copy_required(result_dir / "3_place.odb.def", context["pd_placed_def"], "placed DEF")
    copy_required(report_dir / "4_cts_final.rpt", context["pd_cts_report"], "CTS report")
    copy_required(result_dir / "5_route.odb.def", context["pd_routed_def"], "routed DEF")
    copy_required(result_dir / "6_final.def", context["pd_final_def"], "final DEF")
    copy_required(result_dir / "6_final.gds", context["pd_final_gds"], "final GDS")
    copy_required(result_dir / "6_final.spef", context["pd_final_spef"], "final SPEF")
    copy_required(report_dir / "6_finish.rpt", context["pd_timing_report"], "timing report")

    metrics_path = log_dir / "6_report.json"
    if not metrics_path.is_file():
        raise BuildError(f"missing ORFS final metrics: {metrics_path}")
    metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
    utilization_report = Path(context["pd_utilization_report"])
    utilization_report.write_text(
        "\n".join(
            [
                f"ORFS Nangate45 utilization report for {context['ip']}",
                f"die_area_um2: {metrics.get('finish__design__die__area', 'not_available')}",
                f"core_area_um2: {metrics.get('finish__design__core__area', 'not_available')}",
                f"stdcell_count: {metrics.get('finish__design__instance__count__stdcell', 'not_available')}",
                f"stdcell_area_um2: {metrics.get('finish__design__instance__area__stdcell', 'not_available')}",
                f"utilization: {metrics.get('finish__design__instance__utilization__stdcell', 'not_available')}",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    drc_count = None
    if constraints["run_drc"]:
        drc_count_path = report_dir / "6_drc_count.rpt"
        if not drc_count_path.is_file():
            raise BuildError(f"missing ORFS KLayout DRC count: {drc_count_path}")
        drc_count = int(drc_count_path.read_text(encoding="utf-8").strip())
        Path(context["pd_drc_report"]).write_text(
            "\n".join(
                [
                    f"KLayout Nangate45 DRC report for {context['ip']}",
                    f"violations: {drc_count}",
                    f"status: {'pass' if drc_count == 0 else 'fail'}",
                    f"database: {relativize_path(str(report_dir / '6_drc.lyrdb'))}",
                ]
            )
            + "\n",
            encoding="utf-8",
    ...
    Path(context["pd_lvs_report"]).write_text(
        "\n".join(
            [
                f"KLayout Nangate45 LVS report for {context['ip']}",
                f"requested: {str(constraints['run_lvs']).lower()}",
                "status: not-run",
                f"reason: {constraints['lvs_note']}",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    return {"metrics": metrics, "drc_count": drc_count, "result_dir": result_dir}


def run_pd_exec_backend(context: dict) -> None:
    """Run the pinned ORFS Nangate45 backend when -pd-exec is passed."""
    if not context.get("pd_exec_backend"):
        return
    backend = context["pd_backend"]
    require_keys(
        backend,
        ["platform", "image", "rootfs", "runner"],
        f"profiles.{context['pd_profile']}.backend",
    )
    rootfs = Path(format_text(backend["rootfs"], context)).expanduser()
    runner = Path(format_text(backend["runner"], context)).expanduser()
    openroad = rootfs / "OpenROAD-flow-scripts/tools/install/OpenROAD/bin/openroad"
    if not runner.is_file() or not openroad.is_file():
        raise BuildError(
            f"'-pd-exec' requires the pinned ORFS installation at {rootfs}; run ./setup --pd"
        )
    constraints = validate_orfs_constraints(context)
    orfs_dir = Path(context["pd_out_dir"]) / "orfs"
    config_path, sdc_path, work_home = write_orfs_inputs(context, orfs_dir, constraints)
    config_container = orfs_workspace_path(config_path)
    work_container = orfs_workspace_path(work_home)
    make_base = (
        f"make DESIGN_CONFIG={shlex.quote(config_container)} "
        f"WORK_HOME={shlex.quote(work_container)}"
    )
    followup_targets = ["all_defs"]
    if constraints["run_drc"]:
        followup_targets.append("drc")
    if constraints["run_lvs"]:
        followup_targets.append("lvs")
    inner_command = (
        "source ./env.sh && cd flow && "
        f"{make_base} && {make_base} {' '.join(followup_targets)}"
    )
    command = [
        str(runner),
        "--ro-bind",
    ...
    log_path.write_text(
        "\n".join(
            [
                f"ORFS image: {backend['image']}",
                f"ORFS platform: {backend['platform']}",
                f"ORFS command: {inner_command}",
                result.stdout,
                result.stderr,
            ]
        ),
    ...
    if result.returncode != 0:
        raise BuildError(
            f"ORFS failed with exit code {result.returncode}; see {relativize_path(str(log_path))}"
        )
    captured = capture_orfs_artifacts(context, work_home, sdc_path, constraints)
    context["orfs_metrics"] = captured["metrics"]
    context["orfs_drc_count"] = captured["drc_count"]
    context["orfs_result_dir"] = str(captured["result_dir"])
    metrics = captured["metrics"]
    setup_tns = metrics.get("finish__timing__setup__tns")
    hold_tns = metrics.get("finish__timing__hold__tns")
    if setup_tns != 0 or hold_tns != 0:
        raise BuildError(f"ORFS timing failed: setup TNS={setup_tns}, hold TNS={hold_tns}")
    if constraints["run_drc"] and captured["drc_count"] != 0:
        raise BuildError(f"ORFS KLayout DRC failed with {captured['drc_count']} violations")
...
def run_command(command: str | dict, context: dict, steps_cfg: dict) -> None:
    ...
    if isinstance(command, dict) and command.get("action") == "prepare_pd_signoff_artifacts":
        ...
        return
    if isinstance(command, dict) and command.get("action") == "run_pd_exec_backend":
        run_pd_exec_backend(context)
        return
