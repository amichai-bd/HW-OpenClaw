...
- The [wiki-sync](.github/workflows/wiki-sync.yml) workflow publishes the versioned `wiki/` tree to the GitHub Wiki: it clones `*.wiki.git`, copies and transforms pages (including rewriting links for the Wiki UI), then pushes. Entry point: [`bin/wiki-publish`](bin/wiki-publish) (implementation: [tools/wiki/publish_github_wiki.py](tools/wiki/publish_github_wiki.py)). Manual runs: **Actions → wiki-sync → Run workflow**, or locally after `. cfg/env.sh`: `./bin/wiki-publish --dry-run --output /tmp/wiki-out`.
