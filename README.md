...
- The [wiki-sync](.github/workflows/wiki-sync.yml) workflow publishes the versioned `wiki/` tree to the GitHub Wiki: it clones `*.wiki.git`, copies and transforms pages (including rewriting links for the Wiki UI), then pushes. Implementation: [tools/wiki/publish_github_wiki.py](tools/wiki/publish_github_wiki.py). Manual runs: **Actions → wiki-sync → Run workflow**.
